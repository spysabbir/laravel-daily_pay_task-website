...
class PostingTaskController extends Controller
{
    ...
    public function postTaskStore(Request $request)
    {
        ...
        return to_route('posting_task.list.pending')->with($notification);
    }
    ...
    public function postingTaskListPending(Request $request)
    {
        $user = User::findOrFail(Auth::id());
        $hasVerification = $user->hasVerification('Approved');
        ...
        if (!$hasVerification) {
            ...
            return view('frontend.posting_task.pending');
        }
    }

    public function postingTaskView($id)
    {
        $postTask = PostTask::findOrFail($id);
        return view('frontend.posting_task.view', compact('postTask'));
    }

    public function postingTaskListRejected(Request $request)
    {
        $user = User::findOrFail(Auth::id());
        $hasVerification = $user->hasVerification('Approved');
        ...
        if (!$hasVerification) {
            ...
            return view('frontend.posting_task.rejected');
        }
    }

    public function postingTaskListCanceled(Request $request)
    {
        $user = User::findOrFail(Auth::id());
        $hasVerification = $user->hasVerification('Approved');
        ...
        if (!$hasVerification) {
            ...
            return view('frontend.posting_task.canceled');
        }
    }

    public function postingTaskListPaused(Request $request)
    {
        $user = User::findOrFail(Auth::id());
        $hasVerification = $user->hasVerification('Approved');
        ...
        if (!$hasVerification) {
            ...
            if ($request->ajax()) {
                ...
                return DataTables::of($taskListPaused)
                    ...
                    ->editColumn('action', function ($row) {
                        $btn = '
                            <a href="' . route('running.posting_task.show', encrypt($row->id)) . '" class="btn btn-success btn-xs">Check</a>
    ...
    public function postingRunningTaskCanceled(Request $request, $id)
    {
        $postTask = PostTask::findOrFail($id);
    ...
    public function postingTaskListRunning(Request $request)
    {
        $user = User::findOrFail(Auth::id());
        $hasVerification = $user->hasVerification('Approved');
        ...
        if (!$hasVerification) {
                ...
                return DataTables::of($taskListRunning)
                    ...
                    ->editColumn('action', function ($row) {
                        $btn = '
                            ...
                            <a href="' . route('running.posting_task.show', encrypt($row->id)) . '" class="btn btn-info btn-xs">Check</a>
            ...
            return view('frontend.posting_task.running');
        }
    }

    public function postingRunningTaskEdit(string $id)
    {
        $postTask = PostTask::where('id', $id)->first();
        return response()->json($postTask);
    }

    public function postingRunningTaskUpdate(Request $request, string $id)
    {
        $validator = Validator::make($request->all(), [
            'work_needed' => 'required|numeric|min:1',
        ]);
    ...
    public function postingRunningTaskShow(Request $request, $id)
    {
        if ($request->ajax()) {
        ...
        return view('frontend.posting_task.running_show', compact('postTask', 'proofSubmitted'));
    }

    public function postingRunningTaskApprovedAll($id)
    {
    ...
    public function postingRunningTaskSelectedItemApproved(Request $request)
    {
    ...
    public function postingRunningTaskSelectedItemRejected(Request $request)
    {
        $proofTasks = ProofTask::whereIn('id', $request->id)->get();
    ...
    public function postingRunningTaskProofCheck($id)
    {
        $proofTask = ProofTask::findOrFail($id);
        return view('frontend.posting_task.proof_check', compact('proofTask'));
    }
    ...
    public function postingRunningTaskPausedResume($id)
    {
        $postTask = PostTask::findOrFail($id);
    ...
    public function postingTaskListCompleted(Request $request)
    {
        $user = User::findOrFail(Auth::id());
        $hasVerification = $user->hasVerification('Approved');
        ...
        if (!$hasVerification) {
            ...
            return view('frontend.posting_task.completed');
        }
    }
}
