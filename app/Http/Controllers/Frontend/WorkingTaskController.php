...
class WorkingTaskController extends Controller
{
    ...
    public function workingTaskListPending(Request $request)
    {
        $user = User::findOrFail(Auth::id());
        $hasVerification = $user->hasVerification('Approved');
        ...
        if (!$hasVerification) {
            ...
            return view('frontend.working_task.pending');
        }
    }

    public function workingTaskListApproved(Request $request)
    {
        $user = User::findOrFail(Auth::id());
        $hasVerification = $user->hasVerification('Approved');
        ...
        if (!$hasVerification) {
            ...
            return view('frontend.working_task.approved');
        }
    }

    public function approvedWorkingTaskView($id)
    {
        $proofTask = ProofTask::findOrFail($id);
        $postTask = PostTask::findOrFail($proofTask->post_task_id);
        return view('frontend.working_task.approved_view', compact('proofTask', 'postTask'));
    }

    public function workingTaskListRejected(Request $request)
    {
        $user = User::findOrFail(Auth::id());
        $hasVerification = $user->hasVerification('Approved');
        ...
        if (!$hasVerification) {
            ...
            return view('frontend.working_task.rejected');
        }
    }

    public function rejectedWorkingTaskCheck($id)
    {
        $proofTask = ProofTask::findOrFail($id);
        $postTask = PostTask::findOrFail($proofTask->post_task_id);
        return view('frontend.working_task.rejected_check', compact('proofTask' , 'postTask'));
    }

    public function rejectedWorkingTaskReviewed(Request $request, $id)
    {
        ...
        if ($validator->fails()) {
            ...
            if (30 >= get_default_settings('task_proof_monthly_free_review_time')) {
    ...
    public function workingTaskListReviewed(Request $request)
    {
        $user = User::findOrFail(Auth::id());
        $hasVerification = $user->hasVerification('Approved');
        ...
        if (!$hasVerification) {
            ...
            if ($request->ajax()) {
                ...
                return DataTables::of($taskList)
                    ...
                    ->editColumn('title', function ($row) {
                        return '
                            <a href="'.route('find_task', encrypt($row->post_task_id)).'" title="'.$row->postTask->title.'" class="text-info">
                                '.$row->postTask->title.'
                            </a>
                        ';
                    })
            ...
            return view('frontend.working_task.reviewed');
        }
    }

    public function reviewedWorkingTaskView($id)
    {
        $proofTask = ProofTask::findOrFail($id);
        $postTask = PostTask::findOrFail($proofTask->post_task_id);
        return view('frontend.working_task.reviewed_check', compact('proofTask' , 'postTask'));
    }
}
