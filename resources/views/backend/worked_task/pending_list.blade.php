            ...
            <div class="card-body">
                ...
                <div class="my-3">
                    @php
                        ...
                        $pendingProofStyleWidth = $pendingProofCount != 0 ? round(($pendingProofCount / $proofSubmittedCount) * 100, 2) : 100;
                        $progressBarClass = $proofSubmittedCount == 0 ? 'primary' : 'info';
                    @endphp
                    <p class="mb-1">
                        <strong>Pending Status: </strong>
                        <span class="text-info">Pending: {{ $pendingProofCount }}</span>
                        <span class="text-primary">Submit: {{ $proofSubmittedCount }}</span>,
                    </p>
                    <div class="progress">
                        <div class="progress-bar progress-bar-striped progress-bar-animated bg-{{ $progressBarClass }}" role="progressbar" style="width: {{ $pendingProofStyleWidth }}%" aria-valuenow="{{ $pendingProofCount }}" aria-valuemin="0" aria-valuemax="{{ $proofSubmittedCount }}">{{ $pendingProofCount }} / {{ $proofSubmittedCount }}</div>
                    </div>
                </div>
            </div>
        </div>
