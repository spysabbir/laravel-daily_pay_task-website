    ...
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <h3 class="card-title">Job Post Charge List</h3>
                                        ...
                                        <table id="trashDataTable" class="table w-100">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Working Min Charge</th>
                                                    <th>Working Max Charge</th>
                                                    <th>Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                ...
                                <div class="modal-dialog modal-md">
                                    <div class="modal-content">
                                        ...
                                        <form class="forms-sample" id="editForm">
                                            @csrf
                                            <div class="modal-body">
                                                <input type="hidden" id="job_post_charge_id">
                                                <div class="mb-3">
                                                    <label for="category_id" class="form-label">Category</label>
...
<script>
    ...
    var table = $('#allDataTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('backend.job_post_charge.index') }}",
            data: function(d) {
        ...
        columns: [
            ...
            { data: 'working_min_charge', name: 'working_min_charge' },
            { data: 'working_max_charge', name: 'working_max_charge' },
    ...
    function loadChildCategories(category_id, sub_category_id, target, selectedChildCategory = null) {
        $.ajax({
            url: "{{ route('backend.job_post_charge.get_child_categories') }}",
            type: 'GET',
    ...
    $('#createForm').submit(function(event) {
        ...
        $.ajax({
            url: "{{ route('backend.job_post_charge.store') }}",
            type: 'POST',
            data: $(this).serialize(),
            dataType: 'json',
            beforeSend: function() {
            ...
            success: function(response) {
                if (response.status === 400) {
                    ...
                    $('#createForm')[0].reset();
                    table.ajax.reload();
                    toastr.success('Job Post Charge created successfully.');
                }
            }
        });
    });
    ...
    $(document).on('click', '.editBtn', function() {
        var id = $(this).data('id');
        var url = "{{ route('backend.job_post_charge.edit', ":id") }}".replace(':id', id);
        $.get(url, function(response) {
            $('#job_post_charge_id').val(response.id);
            ...
            $('#working_min_charge').val(response.working_min_charge);
            $('#working_max_charge').val(response.working_max_charge);
        });
    });
    ...
    $('#editForm').submit(function(event) {
        event.preventDefault();
        var id = $('#job_post_charge_id').val();
        var url = "{{ route('backend.job_post_charge.update', ":id") }}".replace(':id', id);
        $.ajax({
            url: url,
            type: "PUT",
            data: $(this).serialize(),
            ...
            success: function(response) {
                if (response.status === 400) {
                    ...
                    table.ajax.reload();
                    toastr.success('Job Post Charge updated successfully.');
                }
            }
        });
    });
    ...
    $(document).on('click', '.deleteBtn', function() {
        var id = $(this).data('id');
        var url = "{{ route('backend.job_post_charge.destroy', ":id") }}".replace(':id', id);
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        toastr.warning('Job Post Charge soft deleted successfully.');
                    }
                });
            }
        });
    ...
    $('#trashDataTable').DataTable({
        ...
        searching: true,
        ajax: {
            url: "{{ route('backend.job_post_charge.trash') }}",
        },
        columns: [
            { data: 'DT_RowIndex', name: 'DT_RowIndex' },
            { data: 'category_name', name: 'category_name' },
    ...
    $(document).on('click', '.restoreBtn', function() {
        var id = $(this).data('id');
        var url = "{{ route('backend.job_post_charge.restore', ":id") }}".replace(':id', id);
        $.ajax({
            url: url,
            type: "GET",
            success: function(response) {
                ...
                table.ajax.reload();
                $('#trashDataTable').DataTable().ajax.reload();
                toastr.success('Job Post Charge restored successfully.');
            }
        });
    });
    ...
    $(document).on('click', '.forceDeleteBtn', function() {
        var id = $(this).data('id');
        var url = "{{ route('backend.job_post_charge.delete', ":id") }}".replace(':id', id);
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(response) {
                        $(".trashModel").modal('hide');
                        $('#trashDataTable').DataTable().ajax.reload();
                        toastr.error('Job Post Charge force deleted successfully.');
                    }
                });
            }
        });
    ...
    $(document).on('click', '.statusBtn', function() {
        var id = $(this).data('id');
        var url = "{{ route('backend.job_post_charge.status', ":id") }}".replace(':id', id);
        $.ajax({
            url: url,
            type: "GET",
            success: function(response) {
                table.ajax.reload();
                toastr.success('Job Post Charge status changed successfully.');
            }
        });
    });
